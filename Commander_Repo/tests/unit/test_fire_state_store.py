...
from core.state.fire_state_store import FireStateStore  # pyright: ignore[reportUnusedImport]
...
class TestFireStateStore:
    ...
    def test_transition_on_terminal_always_rejected(self) -> None:
        ...
        store.get("fire-2")  # pyright: ignore[reportUnusedVariable]
        next(iter(TERMINAL_FIRE_STATES))  # pyright: ignore[reportUnusedVariable]
