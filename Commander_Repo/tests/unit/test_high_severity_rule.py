...
from wfc_shared.enums.capabilities import DISPATCH_COMMANDS  # pyright: ignore[reportUnusedImport]
...
class TestHighSeverityRule:
    def test_triggers_on_high_severity(self, empty_registry: Any) -> None:
        ...
        fire = FireRecord(
            fire_id="fire-high-01",
            state=ACTIVE,
            zone="zone_a",
            severity="HIGH",
            sensor_id="s1",
            location_coords=(36.8, 10.18),
        )
    ...
    def test_no_trigger_for_low_severity(self, empty_registry: Any) -> None:
        ...
        fire = FireRecord(fire_id="fire-low-01", state=ACTIVE, zone="zone_a", severity="LOW", sensor_id="s1")
        rule = HighSeverityRule()
        result = rule.evaluate(fire, reg)
    ...
    def test_no_trigger_for_contained_state(self, empty_registry: Any) -> None:
        ...
        fire = FireRecord(fire_id="fire-cont-01", state=CONTAINED, zone="zone_a", severity="HIGH", sensor_id="s1")
        rule = HighSeverityRule()
        result = rule.evaluate(fire, reg)
        assert not result.triggered
    ...
    def test_no_escalation_target(self, empty_registry: Any) -> None:
        ...
        fire = FireRecord(fire_id="fire-no-esc", state=ACTIVE, zone="zone_a", severity="HIGH", sensor_id="s1")
        rule = HighSeverityRule()
