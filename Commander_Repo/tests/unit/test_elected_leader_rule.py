...
class TestElectedLeaderRule:
    ...
    def fire(self):
        return FireRecord(
            fire_id="fire-el-01", state=ACTIVE, zone="zone_a",
    ...
    def test_rejects_stale_term(self, fire: Any, rule: Any) -> None:
        ...
        reg = NodeRegistry()
        ctx = RuleContext(trigger=EvalTrigger.ELECTION_RESULT, event_log=None,  # pyright: ignore
    ...
    def test_accepts_valid_newer_term(self, rule: Any) -> None:
        from core.node_registry.registry import NodeRegistry
        fire = FireRecord(fire_id="fire-el-02", state=ACTIVE, zone="zone_a",
                          severity="LOW", sensor_id="s1", leader_term=1)
        reg = NodeRegistry()
    ...
    def test_rejects_incapable_node(self, rule: Any) -> None:
        from core.node_registry.registry import NodeRegistry
        fire = FireRecord(fire_id="fire-el-03", state=ACTIVE, zone="zone_a",
                          severity="LOW", sensor_id="s1", leader_term=1)
        reg = NodeRegistry()
    ...
    def test_rejects_fire_id_mismatch(self, rule: Any) -> None:
        from core.node_registry.registry import NodeRegistry
        fire = FireRecord(fire_id="fire-el-04", state=ACTIVE, zone="zone_a",
                          severity="LOW", sensor_id="s1", leader_term=1)
        reg = NodeRegistry()
        reg.register("sl-9", "SWARM_LEADER", [SWARM_LEAD])
        reg.heartbeat("sl-9")
