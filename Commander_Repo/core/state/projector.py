...
class FireProjector:
    ...
    def apply(self, event: DomainEvent) -> None:
        ...
        if handler:
            try:
                ...
                log("FireProjector", f"apply({event.event_type}) fire={event.fire_id} failed: {exc}", channel="SYSTEM")
    ...
    def replay(self, events: list[DomainEvent]) -> None:
        ...
        for event in ordered:
            self.apply(event)
            count += 1
        log("FireProjector", f"replayed {count} domain events on startup", channel="SYSTEM")
    ...
    def _on_fire_dispatched(self, event: DomainEvent) -> None:
        ...
        if fire and node_id not in fire.assigned_nodes:
            self._fires.add_assigned_node(fire_id, node_id, reason="replay_fire_dispatched")
    ...
    def _on_fire_redispatched(self, event: DomainEvent) -> None:
        ...
        if fire and dead_leader in fire.assigned_nodes:
            self._fires.remove_assigned_node(fire_id, dead_leader, reason="replay_fire_redispatched")
