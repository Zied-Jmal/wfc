...
class LeaderLostRule(Rule):
    ...
    def evaluate(self, fire: FireRecord, registry: NodeRegistry, context: RuleContext | None = None) -> RuleResult:
        ...
        backups = [
            ...
            if "LEADER_BACKUP" in (n.capabilities or []) and n.status == "ACTIVE" and n.zone == zone
        ]
        if backups:
            return RuleResult(
                triggered=True,
                ...
                commands=[
                    Command(
                        target_node=backups[0].node_id, command_type=RESPOND_TO_FIRE, payload={"fire_id": fire.fire_id}
                    )
                ],
            )
