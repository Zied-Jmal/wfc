...
from wfc_shared.enums.domain_event_types import ESCALATION_REQUESTED, FIRE_DISPATCHED
...
class RuleEngine:
    ...
    def __init__(
        ...
        gate: ApprovalGate,
        fires: FireStateStore | None = None,
    ...
    def evaluate(self, fire: FireRecord, context: RuleContext | None = None) -> list[RuleResult]:
        ...
        if context.telemetry_rules is not None:
            rules_to_run = [r for r in self._rules if r.name in context.telemetry_rules]
        ...
        for rule in rules_to_run:
            try:
                ...
                if result.triggered:
                    ...
                    if result.state_updates and self._fires is not None:
                        ...
                        if "state" in updates:
                            ...
                            self._fires.transition(fire_id, new_state, reason=f"rule_{rule.name}_{result.reason[:30]}")
                            log(
                                "RuleEngine",
                            ...
                            if updates.get("clear_assigned_nodes"):
                                self._fires.assign_node(
                                    fire_id, None, f"[{rule.name}] cleared assigned_nodes on re-activation"
                                )
                        ...
                        if "preempt_from_fire_id" in updates and "preempt_node_id" in updates:
                            ...
                            self._fires.remove_assigned_node(
                                old_fire_id, stolen_node, f"[{rule.name}] preempted {stolen_node}"
                            )
                            log(
                                "RuleEngine",
                ...
                log("RuleEngine", f"rule '{rule.name}' raised: {exc}", channel="RULES")
    ...
    def _log_result(self, rule_name: str, fire: FireRecord, result: RuleResult) -> None:
        ...
        log(
            "RuleEngine",
            f"[{rule_name}] {status} | fire_id={fire.fire_id[:8]} state={fire.state} reason={result.reason}",
            channel="RULES",
        )
