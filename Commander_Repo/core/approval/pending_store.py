...
from wfc_shared.enums.approval import APPROVED, PENDING, REJECTED
...
class PendingCommandStore:
    ...
    def __init__(
        ...
        event_log: DomainEventLog | None = None,
    ) -> None:
        """
        Args:
    ...
    def add(self, pending: PendingCommand) -> None:
        ...
        if self._ttl is not None and pending.expires_at is None:  # pyright: ignore[reportUnnecessaryComparison]
            pending = pending.model_copy(update={"expires_at": pending.created_at + self._ttl})
        self._store[pending.pending_id] = pending
        ...
        log(
            "PendingStore",
            f"held pending_id={pending.pending_id[:8]} cmd={pending.command.command_type}",
            channel="APPROVAL",
        )
    ...
    def approve(self, pending_id: str, operator_id: str | None = None) -> None:
        ...
        if self._event_log is not None:  # pyright: ignore[reportUnknownMemberType]
            ...
            self._event_log.append(
                DomainEvent(  # pyright: ignore[reportUnknownMemberType]
        ...
        log("PendingStore", f"approved pending_id={pending_id[:8]} trace={trace_id[:8]}", channel="APPROVAL")
    ...
    def reject(
        ...
        log("PendingStore", f"rejected pending_id={pending_id[:8]} reason={reason}", channel="APPROVAL")
    ...
    def expire_stale(self) -> list[str]:
        ...
        expired = [
            ...
            if p.status == PENDING and p.expires_at is not None and now >= p.expires_at
        ]
        for pid in expired:
            ...
            log("PendingStore", f"expired pending_id={pid[:8]} (TTL)", channel="APPROVAL")
        return expired
