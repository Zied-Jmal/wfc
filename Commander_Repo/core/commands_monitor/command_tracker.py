...
class CommandTracker:
    ...
    def create(self, trace_id: str, command: dict[str, Any]) -> None:
        ...
        if trace_id in self._commands:
            log("CommandTracker", f"create() called twice for {trace_id}", channel="TRACKER")
            return
    ...
    def update(
        ...
        if trace_id not in self._commands:
            log("CommandTracker", f"unknown trace_id={trace_id} - dropped", channel="TRACKER")
            return False
        ...
        if event_id is None:
            ...
            log("CommandTracker", f"missing event_id for {trace_id}", channel="TRACKER")
            return False
        # 3. Global dedup
        if event_id in self._seen_event_ids:
            ...
            log("CommandTracker", f"duplicate event_id={event_id}", channel="TRACKER")
            return False
        ...
        if new_state is None:
            ...
            log("CommandTracker", f"unknown event_type={event_type}", channel="TRACKER")
            return False
        # 5. Transition guard
        ...
        if new_state not in allowed:
            ...
            log("CommandTracker", f"invalid transition {current_state}{new_state} for {trace_id}", channel="TRACKER")
            return False
        # 6. Same-state detection
        if new_state == current_state:
            record["history"].append(
                self._make_event(
                    "DUPLICATE_STATE_EVENT",
                    {
                        "state": current_state,
                        "event_type": event_type,
                    },
        ...
        log("CommandTracker", f"{trace_id}: {current_state} {new_state}", channel="TRACKER")
        return True
    ...
    def get_issued(self) -> list[tuple[str, dict[str, Any]]]:
        ...
        """
        return [(tid, rec) for tid, rec in self._commands.items() if rec.get("status") == "ISSUED"]
