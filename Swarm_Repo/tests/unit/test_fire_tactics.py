...
class TestFireTactics:
    def test_assign_respond_to_fire_skips_low_payload(self):
        ...
        assignments = tactics.assign_respond_to_fire(
            "fire-1", (36.80, 10.18), "HIGH",
            [scout], [fighter_ok, fighter_low]
        )
        assigned_ids = [a.drone_id for a in assignments]
        assert "fd-1" in assigned_ids
        assert "fd-2" not in assigned_ids

    def test_orbit_positions_are_evenly_distributed(self):
        from core.tactics.fire_tactics import FireTactics
        tactics = FireTactics()

        scouts = [DroneRecord(drone_id=f"sd-{i}", role="SCOUT", location=(36.8, 10.18))
                  for i in range(4)]
        assignments = tactics.assign_respond_to_fire(
            "fire-1", (36.80, 10.18), "HIGH",
            scouts, []
        )
        scouts_assigned = [a for a in assignments if a.task == "SCOUTING"]
        assert len(scouts_assigned) == 4
        # Verify they're at different positions (not collocated)
        positions = set(a.target_pos for a in scouts_assigned)
    ...
    def test_reassess_low_battery_triggers_returning(self):
        ...
        low_batt_telem = DroneTelemetry(
            drone_id="fd-1", leader_id="sl-1", timestamp=time.time(),
            position=(36.8, 10.18),
            battery_wh=50.0, battery_pct=0.20,
            payload_litres=8.0,
        ...
        fresh_telem = DroneTelemetry(
            drone_id="fd-2", leader_id="sl-1", timestamp=time.time(),
            position=(36.82, 10.20),
            battery_wh=200.0, battery_pct=0.90,
            payload_litres=10.0,
            task="IDLE", connectivity="STRONG",
        )
...
# Fix roles via direct assignment since update_telemetry infers from sensors
        ...
        snap = SwarmStatusSnapshot(
            leader_id="sl-1", fire_id="fire-1", timestamp=time.time(),
            fire_intensity="HIGH",
        )
