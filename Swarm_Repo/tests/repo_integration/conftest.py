...
def mosquitto_broker() -> Generator[tuple[str, int], None, None]:
    ...
    for _ in range(30):
        try:
            c = mqtt.Client()
            c.connect(host, port)  # pyright: ignore[reportUnknownArgumentType]
            c.disconnect()
            break
        except Exception:
            time.sleep(1)
    ...
    yield (host, port)  # pyright: ignore[reportUnknownVariableType, reportUnknownArgumentType]
    subprocess.run(["docker", "rm", "-f", name], capture_output=True)
    os.unlink(conf.name)

@pytest.fixture
def mqtt_client(mosquitto_broker: tuple[str, int]) -> Generator[tuple[mqtt.Client, list[Any], threading.Event], None, None]:
