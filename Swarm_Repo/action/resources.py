...
class DroneResourceModel:
    ...
    def __init__(
        ...
        self._payload_density = RETARDANT_DENSITY_KG_L if payload_type == "retardant" else WATER_DENSITY_KG_L
    ...
    def rtb_level(self) -> str:
        ...
        if frac <= BATTERY_CRITICAL_WH_FRAC:
            return RTBLevel.CRITICAL
        if frac <= RTB_URGENT_WH_FRAC:
            return RTBLevel.URGENT
        if frac <= RTB_WARNING_WH_FRAC:
            return RTBLevel.WARNING
    ...
    def connectivity(self, distance_to_gcs_m: float | None = None) -> str:
        """
        ...
        fspl_db = 20 * math.log10(d) + 20 * math.log10(COMM_FREQ_MHZ * 1e6) - 147.55  # dB
    ...
    def refuel(self, wh: float | None = None) -> None:
        """
    ...
    def reload_payload(self, litres: float | None = None) -> None:
