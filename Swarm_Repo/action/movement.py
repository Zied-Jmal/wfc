...
# Target waypoint in NED
        self._wp_ned: Optional[list[float]] = None
...
# region  PUBLIC API
    ...
    def set_waypoint(self, target: GPSCoord, speed_mps: Optional[float] = None) -> None:
        """
    ...
    def set_waypoint_ned(self, north_m: float, east_m: float, alt_m_amsl: float,
                          speed_mps: Optional[float] = None) -> None:
    ...
    def return_home(self, speed_mps: Optional[float] = None) -> None:
...
# 1. Desired velocity toward waypoint
        if self._wp_ned is not None and not self.is_at_waypoint():
            ...
            v_D_des = max(-MAX_VERTICAL_SPEED_MPS,
                          min( MAX_VERTICAL_SPEED_MPS,
                               dD / max(dt, 1.0)))
        else:
# Hovering
            v_N_des = 0.0
...
# wind vertical (updraft raises drone slightly)
        ...
        self._vel[0] = _clamp(self._vel[0] + a_N * dt,
                               -MAX_HORIZONTAL_SPEED_MPS, MAX_HORIZONTAL_SPEED_MPS)
        self._vel[1] = _clamp(self._vel[1] + a_E * dt,
                               -MAX_HORIZONTAL_SPEED_MPS, MAX_HORIZONTAL_SPEED_MPS)
        self._vel[2] = _clamp(self._vel[2] + a_D * dt,
