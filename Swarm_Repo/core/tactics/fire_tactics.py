...
from wfc_shared.schemas.telemetry import SwarmStatusSnapshot, DroneTelemetry
...
def _dest(origin: tuple[float,float], bearing_deg: float, dist_m: float) -> tuple[float,float]:
    ...
    lat1 = math.radians(origin[0]); lon1 = math.radians(origin[1])
    lat2 = math.asin(math.sin(lat1)*math.cos(ang)
                     + math.cos(lat1)*math.sin(ang)*math.cos(bear))
    lon2 = lon1 + math.atan2(math.sin(bear)*math.sin(ang)*math.cos(lat1),
...
# --- Command-triggered allocation --------------------------
    ...
    def assign_respond_to_fire(
        self,
        fire_id: str, fire_pos: tuple[float,float], severity: str,
        scouts: list[DroneRecord], fighters: list[DroneRecord],
        ...
        for _i, scout in enumerate(scouts):
            assignments.append(DroneAssignment(
                drone_id=scout.drone_id, task="SCOUTING",
                target_pos=fire_pos, severity=severity, priority=8))
        ...
        for fighter in fighters:
            if not self._has_payload(fighter):
                ...
                continue
            assignments.append(DroneAssignment(
                drone_id=fighter.drone_id, task="SUPPRESSING",
                target_pos=fire_pos, severity=severity, priority=9))
            dispatched += 1

        log("FireTactics",
            f"respond_to_fire {fire_id} sev={severity}: "
            f"{len(scouts)} scouts r={orbit_r:.0f}m | {dispatched} fighters",
            channel="SYSTEM")
        return assignments
    ...
    def assign_contain_fire(
        ...
        for i, drone in enumerate(all_drones):
            ...
            if not is_scout and not self._has_payload(drone):
                task = "RETURNING"
            assignments.append(DroneAssignment(
                drone_id=drone.drone_id, task=task,
                target_pos=pos, severity=severity, priority=7))
    ...
    def assign_reinforce(
        self,
        fire_id: str, fire_pos: tuple[float,float], severity: str,
        new_scouts: list[DroneRecord], new_fighters: list[DroneRecord],
        current_count: int, wind_dir_deg: float | None = None,
    ) -> list[DroneAssignment]:
        assignments: list[DroneAssignment] = []
        ...
        for _i, scout in enumerate(new_scouts):
            assignments.append(DroneAssignment(
                drone_id=scout.drone_id, task="SCOUTING",
                target_pos=fire_pos,
                severity=severity, priority=8))

        for fighter in new_fighters:
            if not self._has_payload(fighter): continue
            assignments.append(DroneAssignment(
                drone_id=fighter.drone_id, task="SUPPRESSING",
                target_pos=fire_pos,
                severity=severity, priority=9))
    ...
    def assign_stand_down(self, all_drones: list[DroneRecord]) -> list[DroneAssignment]:
        result: list[DroneAssignment] = [DroneAssignment(drone_id=d.drone_id, task="RETURNING", priority=10)
                for d in all_drones]
        return result
...
# --- Autopilot reassess (every 2s) -------------------------
    ...
    def reassess(
        ...
        scout_telems: list[DroneTelemetry] = [d.last_telemetry for d in drone_registry.get_by_role("SCOUT")
                        if d.last_telemetry]
        ...
        if snapshot.fire_intensity == "CRITICAL":
            scouts_orbiting = any(
                d.last_telemetry and d.last_telemetry.task == "SCOUTING"
                for d in drone_registry.get_by_role("SCOUT")
            )
            if not scouts_orbiting:
                ...
                for drone in all_drones:
                    ...
                    if t and t.task == "IDLE" and drone.role == "FIREFIGHTING":
                        actions.append(DroneAssignment(
                            drone_id=drone.drone_id, task="IDLE", priority=6))

        # Rule 2 - RAPID spread + suppression < 30% -> hot flank
        if (snapshot.spread_rate == "RAPID"
            ...
            for f in candidates:
                actions.append(DroneAssignment(
                    drone_id=f.drone_id, task="SUPPRESSING",
                    target_pos=hot_flank, severity=snapshot.fire_intensity, priority=10))
            if candidates:
                log("FireTactics",
        ...
        for drone in all_drones:
            t = drone.last_telemetry
            if not t or t.task == "RETURNING": continue
            low = (t.battery_pct < RTB_BATTERY_PCT
                   or (t.battery_wh is not None and t.battery_wh < RTB_BATTERY_WH))  # pyright: ignore[reportUnnecessaryComparison]
            if low:
                actions.append(DroneAssignment(
                    drone_id=drone.drone_id, task="RETURNING", priority=9))
                log("FireTactics",
                    f"LOW BATTERY {drone.drone_id}: {getattr(t,'battery_wh',0):.0f} Wh "
                    f"({t.battery_pct:.0%})", channel="SYSTEM")
                ...
                if fresh and fire_pos:
                    ...
                    actions.append(DroneAssignment(
                        drone_id=fresh.drone_id, task=task, target_pos=fire_pos,
                        severity=snapshot.fire_intensity, priority=9))
        ...
        for drone in drone_registry.get_by_role("FIREFIGHTING"):
            ...
            if (t.payload_litres is not None  # pyright: ignore[reportUnnecessaryComparison]
                ...
                actions.append(DroneAssignment(
                    drone_id=drone.drone_id, task="RETURNING", priority=8))
                log("FireTactics",
                    f"LOW PAYLOAD {drone.drone_id}: {t.payload_litres:.2f} L",
                    channel="SYSTEM")
                fresh = self._fresh_idle(drone_registry, "FIREFIGHTING")
                if fresh and fire_pos:
                    actions.append(DroneAssignment(
                        drone_id=fresh.drone_id, task="SUPPRESSING",
                        target_pos=fire_pos, severity=snapshot.fire_intensity, priority=8))

        # Rule 6 - Active fire with idle fighters -> dispatch them
        if (snapshot.suppression_pct is not None
            ...
            if idle_ff:
                actions.append(DroneAssignment(
                    drone_id=idle_ff[0].drone_id, task="SUPPRESSING",
                    target_pos=fire_pos, severity=snapshot.fire_intensity,
                    priority=7))
                log("FireTactics",
        ...
        if (snapshot.suppression_pct is not None
            ...
            active_ff: list[DroneRecord] = [d for d in drone_registry.get_by_role("FIREFIGHTING")
                         if d.last_telemetry and d.last_telemetry.task=="SUPPRESSING"]
            for drone in active_ff[1:]:
                actions.append(DroneAssignment(
                    drone_id=drone.drone_id, task="RETURNING", priority=5))
            if active_ff[1:]:
                log("FireTactics",
                    f"suppression {snapshot.suppression_pct:.0%}: "
                    f"recalling {len(active_ff)-1} surplus", channel="SYSTEM")
...
# --- Advisory ----------------------------------------------
    ...
    def fighters_needed(self, severity: str, perimeter_m: float | None) -> int:
        ...
        l    = _LITRES_PER_M2.get(severity,1.2) * area
        return max(1, min(6, math.ceil(l / 5.0 / 3.0)))
...
# --- Private helpers --------------------------------------
    ...
    def _hot_flank(
        ...
        if wind_dir_deg is None: return fire_pos
    ...
    def _idle_fighters_with_payload(self, registry: DroneRegistry, count: int) -> list[DroneRecord]:
        cands = [d for d in registry.get_idle()
                 if d.role=="FIREFIGHTING" and self._has_payload(d)]
        cands.sort(key=lambda d: (d.last_telemetry.payload_litres or 0.0)
                   if d.last_telemetry else 0.0, reverse=True)
        return cands[:count]
    ...
    def _has_payload(drone: DroneRecord) -> bool:
        ...
        if t.payload_litres is None: return True  # pyright: ignore[reportUnnecessaryComparison]
