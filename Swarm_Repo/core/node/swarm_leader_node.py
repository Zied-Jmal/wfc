...
from core.tactics.fire_tactics import FireTactics, DroneAssignment
...
from wfc_shared.enums.capabilities import (
    SWARM_LEAD, LEADER_BACKUP, RECEIVE_COMMANDS, HEARTBEAT, TELEMETRY,
)
from wfc_shared.enums.command_types import (
    RESPOND_TO_FIRE, CONTAIN_FIRE, STAND_DOWN, REINFORCE_FIRE,
    ABORT_MISSION, REASSIGN_LEADER, CONFIRM_LEADERSHIP,
    RECALL_DRONE, UPDATE_TASK,
)
...
from wfc_shared.schemas.telemetry import (
    DroneTelemetry, SwarmStatusSnapshot, FireIntensityUpdate,
)
...
# Status publish loop (every 10s)
        self._status_thread = threading.Thread(
            target=self._status_publish_loop, daemon=True
        )
...
# Leader heartbeat monitor (backup only)
        if self._is_backup:
            self._monitor_thread = threading.Thread(
                target=self._leader_monitor_loop, daemon=True
            )
            self._monitor_thread.start()

        log("SwarmLeaderNode",
            f"{self.node_id} started (backup={self._is_backup})", channel="SYSTEM")
...
# Check what task the drone was doing before unregistering
                ...
                log("SwarmLeaderNode",
                    f"drone {node_id} went OFFLINE - unregistered", channel="REGISTRY")
                if task in ("SCOUTING", "SUPPRESSING"):
                    log("SwarmLeaderNode",
                        f"lost drone {node_id} had active task {task} - "
                        f"dispatching replacement", channel="REGISTRY")
...
# COMMAND EXECUTION (from commander)
    ...
    def _execute_command(
        ...
        log("SwarmLeaderNode",
            f"executing {command_type}", channel="COMMANDS")
        ...
        if command_type == RESPOND_TO_FIRE:
            ...
            log("SwarmLeaderNode",
                f"reassigned to fire {new_fire_id}", channel="COMMANDS")
...
# Sync current fire state after election win
            ...
            log("SwarmLeaderNode",
                f"leadership confirmed fire={fire_id}", channel="COMMANDS")
...
# Individual command handlers
    ...
    def _cmd_contain_fire(self, payload: dict[str, Any]) -> None:
        ...
        assignments = self._tactics.assign_contain_fire(
            fire_id=fire_id, fire_pos=fire_pos, severity=severity,
            perimeter_m=perimeter_m, scouts=scouts, fighters=fighters,
        )
        self._dispatch_assignments(assignments)
    ...
    def _cmd_reinforce_fire(self, payload: dict[str, Any]) -> None:
        ...
        assignments = self._tactics.assign_reinforce(  # pyright: ignore[reportCallIssue, reportUnknownVariableType]
            fire_id, fire_pos, new_scouts, new_fighters,
            self._drone_reg.count()
        )
...
# DRONE COMMAND DISPATCH
    ...
    def _send_drone_command(self, drone_id: str, command_type: str, payload: dict[str, Any]) -> None:
        ...
        try:
            self.mqtt.publish(command_topic(drone_id), {
                "trace_id": trace_id,
            ...
            log("SwarmLeaderNode",
                f" {command_type} to {drone_id} trace={trace_id[:8]}",
                channel="COMMANDS")
        except UnicodeEncodeError:
            log("SwarmLeaderNode",
                f"failed to send {command_type} to {drone_id} (trace={trace_id[:8]}) - "
                f"payload contains non-UTF8 data", channel="COMMANDS")
# TELEMETRY INGESTION
    def _on_telemetry(self, topic: str, payload: dict[str, Any]) -> None:
        ...
        try:
            ...
            log("SwarmLeaderNode",
                f"bad telemetry payload: {exc}", channel="SYSTEM")
            return
...
# Publish intensity update if changed
                if (
                    snapshot.fire_intensity != prev_intensity
                    and self._current_fire_id
                ):
                ...
                log("SwarmLeaderNode",
                    f"analysis loop error: {exc}", channel="SYSTEM")
...
# STATUS PUBLISH LOOP (every 10s)
    ...
    def _status_publish_loop(self) -> None:
        ...
        while self._running:
            ...
            try:
                ...
                log("SwarmLeaderNode",
                    f"published status snapshot active={snapshot.active_drones} "
                    f"intensity={snapshot.fire_intensity}",
                    channel="SYSTEM")
            except Exception as exc:
                log("SwarmLeaderNode",
                    f"status publish error: {exc}", channel="SYSTEM")
    ...
    def _check_lost_drones(self) -> None:
        ...
        for d in lost:
            ...
            if task in ("SCOUTING", "SUPPRESSING"):
                log("SwarmLeaderNode",
                    f"drone {d.drone_id} lost {now-lost_since:.0f}s (task={task}) - "
                    f"unregistering + replacing", channel="SYSTEM")
                ...
                self._lost_tracker.pop(d.drone_id, None)
                log("SwarmLeaderNode",
                    f"drone {d.drone_id} lost (no active task) - unregistered",
                    channel="SYSTEM")
    ...
    def _replace_lost_drone(self, lost_id: str, role: str, task: str) -> None:
        ...
        if not self._current_fire_pos:
            log("SwarmLeaderNode",
                f"cannot replace {lost_id}: no current fire position known",
                channel="SYSTEM")
            return
        ...
        if replacement:
            self._send_drone_command(
                replacement.drone_id, UPDATE_TASK,
                {"task": task, "target_pos": list(self._current_fire_pos)}
            )
            log("SwarmLeaderNode",
                f"dispatched {replacement.drone_id} as replacement for lost "
                f"{lost_id} (task={task})", channel="SYSTEM")
        else:
            log("SwarmLeaderNode",
                f"no idle {role} available to replace lost {lost_id}",
                channel="SYSTEM")
...
# FIRE EVENT PUBLISHING
    ...
    def publish_fire_rekindled(self, fire_id: str, location: tuple[float, float]) -> None:
        ...
        self.mqtt.publish(FIRE_REKINDLED_TOPIC, {
            "fire_id":    fire_id,
            "leader_id":  self.node_id,
            "timestamp":  time.time(),
    ...
    def publish_fire_verified(self, fire_id: str, location: tuple[float, float], intensity: str) -> None:
        ...
        self.mqtt.publish(FIRE_VERIFIED_TOPIC, {
            "fire_id":    fire_id,
            "leader_id":  self.node_id,
            "timestamp":  time.time(),
            "location":   list(location),
...
# LEADER HEARTBEAT MONITOR (backup only)
    ...
    def set_current_leader(self, leader_id: str, term: int = 1) -> None:
        ...
        log("SwarmLeaderNode",
            f"current leader set to {leader_id} (term={term})", channel="SYSTEM")
    ...
    def _leader_monitor_loop(self) -> None:
        ...
        while self._running:
            ...
            if elapsed > self._hb_timeout and not self._election_state.in_election:
                log("SwarmLeaderNode",
                    f"leader {self._current_leader_id} timed out after {elapsed:.1f}s - "
                    f"starting election",
                    channel="SYSTEM")
...
# ELECTION CALLBACKS
    ...
    def _on_election_win(self) -> None:
        ...
        log("SwarmLeaderNode",
            f"WON election term={term} - promoting to SWARM_LEAD", channel="SYSTEM")
        ...
        log("SwarmLeaderNode",
            "election win - resuming fire operations immediately", channel="SYSTEM")

    def _on_election_lost(self) -> None:
        """Called by LeaderElection when another node wins - remain backup."""
        log("SwarmLeaderNode",
            "election lost - remaining backup", channel="SYSTEM")
