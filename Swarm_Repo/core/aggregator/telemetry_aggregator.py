...
class TelemetryAggregator:
    ...
    def snapshot(self) -> SwarmStatusSnapshot:
        ...
        fighters = [
            ...
            and t.payload_litres >= 0.0
            and t.task in ("SUPPRESSING", "RETURNING", "IDLE")
        ]

        # Battery (V2: Wh + pct)
        avg_battery_pct = sum(t.battery_pct for t in active) / len(active) if active else 0.0
        ...
        # Payload (V2: litres)
        pl_vals = [t.payload_litres for t in fighters if t.payload_litres is not None]  # pyright: ignore[reportUnnecessaryComparison]
        avg_payload_litres = sum(pl_vals) / len(pl_vals) if pl_vals else 0.0

        # Total litres delivered (V2)
        total_litres = sum(t.litres_delivered for t in active if t.litres_delivered is not None)
    ...
    def _calc_fire_intensity(self, scouts: list[DroneTelemetry]) -> str:
        ...
        """
        temps = sorted(
            [t.thermal_peak_temp_c for t in scouts if t.thermal_peak_temp_c is not None],
            reverse=True,
        )
        if not temps:
            return "LOW"
        ...
        if peak >= _THRESH_CRITICAL_C:
            return "CRITICAL"
        if peak >= _THRESH_HIGH_C:
            return "HIGH"
        if peak >= _THRESH_MEDIUM_C:
            return "MEDIUM"
        return "LOW"
    ...
    def _best_perimeter(self, scouts: list[DroneTelemetry]) -> float | None:
        ...
        """
        vals = [
            t.perimeter_estimate_m for t in scouts if t.perimeter_estimate_m is not None and t.perimeter_estimate_m > 0
        ]
        if vals:
    ...
    def _convex_hull(self, pts: list[tuple[float, float]]) -> list[tuple[float, float]]:
        ...
        while True:
            ...
            cand = pts[0]
            for p in pts[1:]:
                cross = (cand[0] - cur[0]) * (p[1] - cur[1]) - (cand[1] - cur[1]) * (p[0] - cur[0])
                if cross > 0 or (
                    cross == 0 and _hav(p[0], p[1], cur[0], cur[1]) > _hav(cand[0], cand[1], cur[0], cur[1])
                ):
                    cand = p
            if cand == hull[0]:
                break
    ...
    def _calc_spread_rate(self, history: dict[str, list[tuple[float, DroneTelemetry]]], now: float) -> str | None:
        """
    ...
    def _calc_wind(self, scouts: list[DroneTelemetry]) -> tuple[float | None, float | None]:
        """
        Time-weighted average wind from scouts (exponential decay tau=10s).
        More recent readings contribute more.
        """
        speed_vals = [(t.wind_speed_mps, t.timestamp) for t in scouts if t.wind_speed_mps is not None]
        dir_vals = [(t.wind_direction_deg, t.timestamp) for t in scouts if t.wind_direction_deg is not None]
        if not speed_vals:
            return (None, None)
        now = time.time()
        tau = 10.0
