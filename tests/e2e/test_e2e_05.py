...
class TestNodeLifecycleLWT:
    ...
    def test_graceful_shutdown_publishes_offline(
        ...
        env: dict[str, str] = {**os.environ, "MQTT_HOST": host, "MQTT_PORT": str(port),
                               ...
                               "IS_BACKUP": "false", "DEBUG": "0",
                               "PYTHONPATH": f"{swarm};{shared}"}
        proc: subprocess.Popen[bytes] = subprocess.Popen([sys.executable, "main_leader.py"],
                                                         cwd=swarm, env=env,
        ...
        online_msgs_before: list[tuple[str, Any]] = [(t, p) for t, p in all_msgs
                                                      if t == f"wfc/registry/announce/{node_id}"
        ...
        offline_msgs: list[tuple[str, Any]] = [(t, p) for t, p in all_msgs
                                                if t == f"wfc/registry/announce/{node_id}"
                                                and p.get("status") == "OFFLINE"]

        assert len(offline_msgs) >= 1, \
            f"Node {node_id} did not publish OFFLINE on shutdown. " \
            f"Messages after kill: {all_msgs}"
