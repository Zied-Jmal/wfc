...
class TestCommanderDashboardEvents:
    ...
    def test_dashboard_shows_cmd_events(
        ...
        dash_proc, dash_port = dashboard_process
        ...
        events: Any = r.json()
        event_types: list[str] = [e.get("type") for e in (events if isinstance(events, list) else events.get("events", []))]
        assert "ANNOUNCE" in event_types, f"no ANNOUNCE event: {event_types}"
        assert "ACK" in event_types or "CMD_SENT" in event_types, \
            f"no ACK/CMD_SENT events. All types: {event_types}"
