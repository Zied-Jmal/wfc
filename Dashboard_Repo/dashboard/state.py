...
from wfc_shared.enums.fire_status     import ACTIVE as FIRE_ACTIVE, SUPPRESSED, CONTAINED
from wfc_shared.enums.events          import FIRE_SUPPRESSED, FIRE_CONTAINED
...
class NodeState:
    ...
    __slots__ = (
        # Identity
        "node_id", "node_type", "capabilities", "status", "zone",
        ...
        "active_drones", "lost_drones",
        "avg_battery_pct",       # 0.0-1.0
        "min_battery_wh",        # Wh
        "avg_payload_litres",    # L
        "total_litres_delivered",# L
        "fire_id", "fire_intensity", "swarm_status",
        "suppression_pct", "spread_rate", "perimeter_estimate",
        "wind_speed_mps_snap", "wind_direction_deg_snap",  # from snapshot
        # Commander snapshot
        "is_active_commander", "commander_term",
    )
...
class FireRecord:
    ...
    __slots__ = (
        "fire_id", "zone", "severity", "sensor_id",
        "location_coords",  # [lat_deg, lon_deg] WGS-84 or None
        "event_type", "source", "fire_status",
        "fire_intensity", "perimeter_m", "spread_rate",
        "wind_speed_mps", "leader_id", "ts",
    )
