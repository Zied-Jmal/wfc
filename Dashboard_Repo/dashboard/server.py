...
from wfc_shared.enums.command_types import (
    RESPOND_TO_FIRE, CONTAIN_FIRE, STAND_DOWN,
    REINFORCE_FIRE, ABORT_MISSION, REASSIGN_LEADER, CONFIRM_LEADERSHIP,
    DISPATCH_DRONE, RECALL_DRONE, UPDATE_TASK,
)
...
def _ensure_pub() -> None:
    if not getattr(_pub, "_ok", False):
        try:
            ...
            raise HTTPException(503, f"MQTT broker unreachable: {exc}")
...
def _send_to_leader(leader_id: str, command_type: str, extra: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    swarm_state.add_event({"type": "CMD_SENT", "target": leader_id,
                           "command": command_type, "trace_id": cmd["trace_id"][:8]})
...
async def fire_respond(req: FireRequest) -> dict[str, Any]:
    ...
    return _send_to_leader(req.leader_id, RESPOND_TO_FIRE,
                           {"fire_id": fire_id, "location": req.location})

@app.post("/api/fire/contain")
async def fire_contain(req: FireRequest) -> dict[str, Any]:
    return _send_to_leader(req.leader_id, CONTAIN_FIRE,
                           {"fire_id": req.fire_id, "location": req.location})
...
async def drone_command(req: DroneCmd) -> dict[str, Any]:
    ...
    swarm_state.add_event({"type": "CMD_SENT", "target": req.drone_id,
                           "command": req.command_type, "trace_id": cmd["trace_id"][:8]})
    return {"ok": True, "trace_id": cmd["trace_id"]}
...
async def approval_respond(req: ApprovalResp) -> dict[str, Any]:
    ...
    swarm_state.add_event({"type": "APPROVAL_RESP",
                           "request_id": req.request_id[:8], "approved": req.approved})
    return {"ok": True}
...
rule engine FireDispatchRule RESPOND_TO_FIRE to nearest leader.
    ...
    swarm_state.add_event({
        "type": "FIRE_SENSOR_INJECT", "fire_id": fire_id,
        "zone": req.zone, "severity": req.severity,
    })
    return {"ok": True, "fire_id": fire_id, "trigger": "sensor",
            "topic": EVENTS_FIRE}
...
async def inject_fire_intensity(req: InjectFireIntensity) -> dict[str, Any]:
    ...
    swarm_state.add_event({
        "type": "FIRE_INTENSITY_INJECT", "fire_id": fire_id,
        "leader_id": req.leader_id, "intensity": req.new_intensity,
    })
    return {"ok": True, "fire_id": fire_id, "trigger": "leader_intensity",
            "topic": FIRE_INTENSITY}
